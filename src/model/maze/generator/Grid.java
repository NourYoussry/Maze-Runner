...
import model.cells.bombs.BombFactory;
import model.cells.characters.monsters.MonsterFactory;
    ...
    public Cell[][] toCellMatrix (char[][] charMatrix) {
        ...
        for (int row = 0; row < 2 * this.rows + 1; row++) {
            for (int column = 0; column < 2 * this.columns + 1; column++) {
                ...
                /* If the character cell represents a monster. */
                else if (charMatrix[row][column] == 'z' || charMatrix[row][column] == 'x')
                    cellMatrix[row][column] = new MonsterFactory().getMonster(charMatrix[row][column]);
